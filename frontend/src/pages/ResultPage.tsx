    ...
    <div className="w-full h-screen flex flex-col items-center justify-start pt-12 px-6 bg-black/90 animate-in fade-in duration-500 overflow-y-auto">
      <div className="w-full max-w-md flex flex-col items-center gap-6 pb-12">
        ...
        <div className="text-center">
          ...
          {passed && myRank && (
            <p className="text-white/50 text-sm">Ranked <span className="text-white font-bold">#{myRank}</span> on the leaderboard</p>
          )}
        </div>

        {/* Prompt + letter count */}
        <div className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col gap-2">
          <p className="text-white/40 text-xs font-semibold uppercase tracking-widest">Your prompt</p>
          ...
          <div className="flex items-center justify-between pt-1 border-t border-white/10 mt-1">
            <span className="text-white/40 text-xs">Letter count</span>
        ...
        {!passed && failed.length > 0 && (
          <div className="w-full flex flex-col gap-2">
            <p className="text-white/40 text-xs font-semibold uppercase tracking-widest">Criteria not met</p>
            {failed.map((c) => (
        ...
        {leaderboard.length > 0 && (
          <div className="w-full bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
            <div className="px-4 py-3 border-b border-white/10">
              <span className="text-white/60 text-xs font-semibold uppercase tracking-widest">Leaderboard</span>
            </div>
